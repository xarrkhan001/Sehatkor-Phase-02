...
const ServiceWhatsAppButton = ({ phoneNumber, serviceName, providerName, onChatClick, providerId, serviceId, className }: ServiceWhatsAppButtonProps) => {
  if (!phoneNumber) {
    return null;
  }
      ...
      <Tooltip open={open} onOpenChange={isSmallScreen ? undefined : setOpen}>
        <TooltipTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            className={className || "rounded-full w-8 h-8 p-[1px] bg-gradient-to-br from-sky-500/35 via-violet-500/35 to-fuchsia-500/35 hover:from-sky-500/50 hover:via-violet-500/50 hover:to-fuchsia-500/50 transition-colors"}
          ...
          >
            <span className={`flex items-center justify-center w-full h-full bg-background text-blue-600 hover:text-blue-700 ${className?.includes('rounded-2xl') ? 'rounded-2xl' : 'rounded-full'}`}>
              <MessageCircle className={`${className?.includes('h-14') ? 'w-6 h-6' : 'w-4 h-4'}`} />
            </span>
          </Button>
        </TooltipTrigger>
        <TooltipContent
