...
const DoctorsPage = () => {
  ...
  useEffect(() => {
    ...
    const filtered = all.filter(
      (service: Service) => service.type === "Treatment" || service.type === "Surgery"
    );
    ...
    setDoctorServices(filtered as any);
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'sehatkor_services') {
        ...
        const filtered2 = all2.filter(
          (s: Service) => s.type === "Treatment" || s.type === "Surgery"
        );
        ...
        setDoctorServices(filtered2 as any);
      }
    };
