...
export const deleteDoctorService = async (req, res) => {
  try {
    const { id } = req.params;
    const doc = await DoctorService.findOneAndDelete({ _id: id, providerId: req.userId });
    if (!doc) return res.status(404).json({ message: 'Service not found' });
    try {
      if (doc.imagePublicId) await cloudinary.uploader.destroy(doc.imagePublicId, { resource_type: 'image' });
    } catch {}
