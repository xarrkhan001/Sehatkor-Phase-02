...
export const listClinicServices = async (req, res) => {
  try {
    const services = await ClinicService.find({ providerId: req.userId }).sort({ createdAt: -1 });
...
export const createClinicService = async (req, res) => {
  try {
    const { name, description, price, department, category, duration, imageUrl, imagePublicId, providerName } = req.body || {};
...
export const deleteClinicService = async (req, res) => {
  try {
    const { id } = req.params;
    const doc = await ClinicService.findOneAndDelete({ _id: id, providerId: req.userId });
    if (!doc) return res.status(404).json({ message: 'Service not found' });
    try { if (doc.imagePublicId) await cloudinary.uploader.destroy(doc.imagePublicId, { resource_type: 'image' }); } catch {}
